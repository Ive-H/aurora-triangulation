...
center_pt_110km_Lapua = zeros(n_shapes,T,3);
center_pt_110km_Juva  = zeros(n_shapes,T,3);
parspeeds_110km_Lapua = zeros(n_shapes,T-1);
parspeeds_110km_Juva  = zeros(n_shapes,T-1);
...
for t = 1:T
    ...
    for i = (1:N)
        ...
        psi   = rad(ori{t}.p_deg(i), ori{t}.p_min(i), ori{t}.p_sec(i));
        lambda= rad(ori{t}.l_deg(i), ori{t}.l_min(i), ori{t}.l_sec(i));
        ...
        if i <= N/2
            ...
            center_pt_110km_Lapua(s,t,:) = [X(L), Y(L), Z(L)];
        end

        if i > N/2
            s = ori{t}.bulb_number(i); % shape index number
            center_pt_110km_Juva(s,t,:) = [X(L), Y(L), Z(L)];
        end
    end
    ...
    for s = 1:n_shapes
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % scatter3(x,     y,     z,     20, [1 0 0], 'filled'); % clr(ori{t}.bulb_number(i),:), 'filled');  % clr(t,:)
...
for s = 1:n_shapes
    ...
    total_displacement_110km_Lapua = squeeze(center_pt_110km_Lapua(s,end_idx(s),:) - center_pt_110km_Lapua(s,start_idx(s),:));
    total_distance_110km_Lapua = norm(total_displacement_110km_Lapua);
    
    total_displacement_110km_Juva = squeeze(center_pt_110km_Juva(s,end_idx(s),:) - center_pt_110km_Juva(s,start_idx(s),:));
    total_distance_110km_Juva = norm(total_displacement_110km_Juva);
    ...
    for t = start_idx(s):end_idx(s)-1
        ...
        parspeeds_110km_Lapua(s,t) = dot(...
            squeeze(center_pt_110km_Lapua(s,t+1,:)-center_pt_110km_Lapua(s,t,:)) ,...
            total_displacement_110km_Lapua...
          ) / total_distance_110km_Lapua / dt(t);
        
        parspeeds_110km_Juva(s,t) = dot(...
            squeeze( center_pt_110km_Juva(s,t+1,:)- center_pt_110km_Juva(s,t,:)) ,...
            total_displacement_110km_Juva...
          ) / total_distance_110km_Juva / dt(t);
    end



    % f = figure;
    % f.Position = positions(s,:);
    % surf(px(th,ph), py(th,ph), pz(th), map_image, "LineStyle", "none");
    % % plots a section of the Earth's surface
    % xlabel("x (km)");
    % ylabel("y (km)");
    ...
    % hold on;    
    % 
    % scatter3(center_pt(s,:,1), center_pt(s,:,2), center_pt(s,:,3), 30, clr(ori{t}.bulb_number(i),:), 'filled');
    % scatter3(center_pt_110km_Lapua(s,:,1), center_pt_110km_Lapua(s,:,2), center_pt_110km_Lapua(s,:,3), 20, [1 0 0], 'filled');
    % scatter3( center_pt_110km_Juva(s,:,1), center_pt_110km_Juva(s,:,2), center_pt_110km_Juva(s,:,3), 20, [.5 0 .5], 'filled');
    % hold off;
end
...
%     en_t = (en-1)*dt(1);
% 
%     t_start_utc     = datetime(2025,4,3,21,5,33 + 17*st);
%     t_end_utc       = datetime(2025,4,3,21,5,33 + 17*en);
...
positions_main  =  [2110  327 560 420;...
                    2677  327 560 420;...
                    1879 -174 560 420;...
                    2854 -169 560 420;];
positions_small =  [3281  335 560 420;...
                    2308 -170 560 420;...
                    3401 -165 560 420;];

for i = 1:4
    f = figure;
    f.Position = positions_main(i,:);
    hold on;
    ...
    plot(x, parspeeds_110km_Lapua(s,st:en-1), "Color", [1 0 0]);
    plot(x, parspeeds_110km_Juva( s,st:en-1), "Color", [.5 0 .5]);
    ...
    legend(["", "original", "Lapua-110km", "Juva-110km", ""]);
    grid on;
    hold off;
end
...
for i = 1:3
    ...
    t_start_utc_2     = datetime(2025,4,3,21,5,33 + 17*st + 17/2);
    t_end_utc_2       = datetime(2025,4,3,21,5,33 + 17*en - 17/2);
