...
% for each timestep.
%
% near identical to plotting8, with edits for plotting ./sticks/..
% just so stuff doesn't need to be commented out and changed 
% every time I want to switch between datasets.
...
% ori{1} = readtable("orientations/bulb_centers/sticks/5_33-16.csv");   % 1,2,3,4, ,6
% ori{2} = readtable("orientations/bulb_centers/sticks/7_32-16.csv");   % 1,2,3,4,5,6,7
% ori{3} = readtable("orientations/bulb_centers/sticks/10_39-16.csv");  % 1,2, ,4, ,6
% T = 3;
% start_idx = [  1,  1,  1];
% end_idx   = [  3,  3,  3];
% n_shapes = 3;

ori{1} = readtable("orientations/bulb_centers/sticks/5_33-16.csv");
ori{2} = readtable("orientations/bulb_centers/sticks/5_50-16.csv");
ori{3} = readtable("orientations/bulb_centers/sticks/6_07-16.csv");
ori{4} = readtable("orientations/bulb_centers/sticks/6_24-16.csv");
ori{5} = readtable("orientations/bulb_centers/sticks/6_41-16.csv");
ori{6} = readtable("orientations/bulb_centers/sticks/6_58-16.csv");
ori{7} = readtable("orientations/bulb_centers/sticks/7_15-16.csv");
ori{8} = readtable("orientations/bulb_centers/sticks/7_32-16.csv");
ori{9} = readtable("orientations/bulb_centers/sticks/7_49-16.csv");
ori{10} = readtable("orientations/bulb_centers/sticks/8_06-16.csv");
ori{11} = readtable("orientations/bulb_centers/sticks/8_23-16.csv");
ori{12} = readtable("orientations/bulb_centers/sticks/8_40-16.csv");
ori{13} = readtable("orientations/bulb_centers/sticks/8_57-16.csv");
ori{14} = readtable("orientations/bulb_centers/sticks/9_14-16.csv");
ori{15} = readtable("orientations/bulb_centers/sticks/9_31-16.csv");
ori{16} = readtable("orientations/bulb_centers/sticks/9_48-16.csv");
ori{17} = readtable("orientations/bulb_centers/sticks/10_05-16.csv");
ori{18} = readtable("orientations/bulb_centers/sticks/10_22-16.csv");
ori{19} = readtable("orientations/bulb_centers/sticks/10_39-16.csv");
T = 19;
start_idx = [  1,  1,  1];
end_idx   = [ 19, 19, 19];
n_shapes = 3;
...
clr = [1 0 0;   0 1 0; 0 0 1]; %.6 .4 0]; % 0 .6 .4; .4 0 .6; .6 .6 .6;];
dt = ones(T-1,1)*17;
...
inter_bulb_distance = zeros(2,T); % only bulbs [1 2 4 6]
error_inter_distance= zeros(2,T);
...
for t = 1:T
    ...
    for s = 1:n_shapes
        ...
        error_theta(s,t) = error_z_div_r / (1 - (z/r)^2);
...
% daspect([1 1 1])
% legend(["", "1", "2", "4"]);%, "Position", [0.837,0.321,0.065,0.198]);
% 
% hold off;
...
avg_heights              = mean(heights,2);
avg_error_position       = mean(error_position,2);

avg_parallel_speeds      = mean(parallel_speeds,2);
avg_error_speeds         = mean(error_speeds,2);
...
ylim([0 ymax]);
xlabel("time (UTC)")
ylabel("distance between sticks (km)")
legend([""; "D_{1-2}"; ""; "D_{2-4}"]);
grid on;
hold off;
...
%     line_names3 = cat(1,line_names3, int2str(sti(s)));
% end
% xlabel("Longitude");
% ylabel("Latitude");
% xlim([21 37])
...
% parallel_speeds = cat(1,parallel_speeds',[0 0 0]);
% error_speeds    = cat(1,error_speeds',[0 0 0]);
% inter_bulb_distance = inter_bulb_distance';
% error_inter_distance = error_inter_distance';
% 
% writetable(table(heights, error_position, parallel_speeds, error_speeds, inter_bulb_distance, error_inter_distance), "sticks.csv");